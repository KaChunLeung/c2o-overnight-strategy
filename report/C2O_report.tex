...
\newcommand{\icBase}{0.017}
\newcommand{\icHGB}{0.023}
\newcommand{\icEns}{0.022}
\newcommand{\icEnsT}{7.1}
\newcommand{\spreadEns}{4.6}
\newcommand{\grossSharpeBench}{1.78}
\newcommand{\netSmall}{0.25}
\newcommand{\netBench}{0.39}
\newcommand{\netLarge}{0.37}
\newcommand{\posYears}{7/10}
...
$\approx\icHGB$ ($t\approx7.9$), ensemble $\approx\icEns$ ($t\approx\icEnsT$, positive on $\approx58\%$ of
...
\midrule
10\% (decile) & 1.53 & $-1.01$ & $-4.1$ & $-34.9$ & 1.00\\
5\%          & 1.51 & $-0.49$ & $-2.5$ & $-24.4$ & 0.99\\
3\%          & 1.65 & $+0.03$ & $+0.2$ & $-13.7$ & 0.84\\
\textbf{2\% (headline)} & \textbf{1.78} & $\mathbf{+0.39}$ & $\mathbf{+1.9}$ & $-7.2$ & 0.65\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\$50M  & $+1.5\%$ & $6.0\%$ & $\mathbf{+\netSmall}$ & 1.86 & $-12.8\%$ & 0.93 & 0.93\\
\$250M & $+1.9\%$ & $4.8\%$ & $\mathbf{+\netBench}$ & 1.78 & $-7.2\%$  & 0.65 & 0.65\\
\$1B   & $+0.8\%$ & $2.3\%$ & $\mathbf{+\netLarge}$ & 1.40 & $-4.5\%$  & 0.23 & 0.23\\
\bottomrule
\end{tabular}
\end{table}
\noindent As AUM rises the $2\%$ book cannot absorb the capital under the $5\%$ ADV cap, so gross utilisation
falls ($0.93\!\to\!0.65\!\to\!0.23$) while every position is pinned at the $5\%$ cap; Sharpe is preserved
because cost scales with deployed gross. This is the capacity story made explicit.

\subsection{Q6.3 --- gross$\to$net decomposition}
The degradation is almost entirely \emph{slippage} ($1.5\,\text{bps}\times2\ \text{legs}=3$\,bps/night of
gross), then \emph{commission} ($1$\,bps/night); \emph{borrow is negligible} ($\approx0.01$--$0.05$\,bps of
AUM/day) because Tier-C is excluded and Tier-B is a small minority. Table \code{step5\_gross\_to\_net\_decomposition}
reports the Sharpe lost at each stage (gross $\approx1.78\to$ net $\approx0.39$ at \$250M).
...
The book is roughly market-neutral by construction. We report behaviour in 2018\,Q4, 2020\,Q1 (COVID) and
full-2022 in \code{step5\_stress\_windows.csv}; the main dislocation risk is short-side squeeze/borrow stress,
mitigated by Tier-C exclusion and the volatility-floor filter (which thins the eligible set in 2020\,Q1, so the
strategy deploys less gross precisely when impact is worst).
